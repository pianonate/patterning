...
class BoundingBox(bounds: Bounds, cellSize: BigDecimal) {
    ...
    val right =
        if (leftWithOffset + widthDecimal > LifePattern.canvasWidth) LifePattern.canvasWidth.toFloat() + positiveOffScreen else rightFloat
    val bottom =
        if (topWithOffset + heightDecimal > LifePattern.canvasHeight) LifePattern.canvasHeight.toFloat() + positiveOffScreen else bottomFloat
    ...
    private val horizontalLine: Line
        get() {
            ...
            val startY = when {
                ...
                startYDecimal > LifePattern.canvasHeight -> LifePattern.canvasHeight.toFloat() + 1
                else -> startYDecimal.toFloat()
            }
            val endXDecimal = leftWithOffset + widthDecimal
            val endX =
                if (endXDecimal > LifePattern.canvasWidth) LifePattern.canvasWidth.toFloat() + 1 else endXDecimal.toFloat()
    ...
    private val verticalLine: Line
        get() {
            ...
            val startX = when {
                ...
                startXDecimal > LifePattern.canvasWidth -> LifePattern.canvasWidth.toFloat() + 1
                else -> startXDecimal.toFloat()
            }
            val endYDecimal = topWithOffset + heightDecimal
            val endY =
                if (endYDecimal > LifePattern.canvasHeight) LifePattern.canvasHeight.toFloat() + 1 else endYDecimal.toFloat()
